...
export function ReleaseChannelSection(): React.JSX.Element {
  ...
  // downloads against its own baked-in publisher name; macOS installs only a bundle
  // signed like the running app, so a fork build cannot reach the primary source's
  // builds — which are the ones this picker lists — through the updater.
  const needsManualInstall = requiresManualInstall({
    platform,
    running: {
      source: appVersion === null ? null : getVersionReleaseSource(appVersion),
